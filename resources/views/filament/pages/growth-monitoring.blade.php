    ...
    <div class="sickas-page space-y-6">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div class="bg-gradient-to-r from-amber-50 via-white to-emerald-50 px-5 py-5 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
                <p class="text-sm font-medium text-amber-700 dark:text-amber-300">Operasional Ternak</p>
                <div class="mt-1 flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <h1 class="text-2xl font-semibold tracking-tight text-gray-950 dark:text-white">Monitoring Pertumbuhan Ternak</h1>
                        <p class="mt-1 max-w-3xl text-sm text-gray-600 dark:text-gray-300">
                            Pantau performa penggemukan, peringatan timbang, ADG, dan indikator jual per batch atau per ekor.
                        </p>
                    </div>
                    <x-filament::badge color="warning">
                        Data timbang berkala
                    </x-filament::badge>
                </div>
            </div>
        </div>
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ $this->monitoringMode === 'individual' ? 'Ternak Terpantau' : 'Batch Terpantau' }}</p>
                <p class="mt-2 text-2xl font-semibold text-gray-950 dark:text-white">{{ $summary['total_batches'] }} {{ $this->monitoringMode === 'individual' ? 'ekor' : 'batch' }}</p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $this->monitoringMode === 'individual' ? 'Ternak aktif yang masuk monitoring.' : 'Batch aktif yang masuk monitoring.' }}</p>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Rata-rata kenaikan harian batch.</p>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Bagus</p>
                <p class="mt-2 text-2xl font-semibold text-green-600 dark:text-green-400">{{ $summary['good'] }} batch</p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">ADG sudah sesuai target.</p>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Turun</p>
                <p class="mt-2 text-2xl font-semibold text-red-600 dark:text-red-400">{{ $summary['down'] }} batch</p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Perlu cek kesehatan dan pakan.</p>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Belum Ditimbang</p>
                <p class="mt-2 text-2xl font-semibold text-yellow-600 dark:text-yellow-400">{{ $summary['not_weighed'] }} batch</p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Belum punya data timbang awal.</p>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <p class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Perlu Timbang Ulang</p>
                <p class="mt-2 text-2xl font-semibold text-orange-600 dark:text-orange-400">{{ $summary['needs_reweighing'] }} batch</p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Timbang terakhir lewat 14 hari.</p>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div class="mb-4 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-base font-semibold text-gray-950 dark:text-white">Filter Monitoring</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Saring berdasarkan batch, kandang, status pertumbuhan, atau tanggal mulai.</p>
                </div>
            </div>
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
                <label class="space-y-1">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-200">Tampilan</span>
                    <select
                        wire:model.live="monitoringMode"
                    ...
                    >
                        ...
                        <option value="individual">Per Ekor</option>
                    </select>
                </label>
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="flex flex-col items-end gap-1">
                            ...
                            <x-filament::badge :color="$this->weighingAlertColor($row['weighing_alert_status'])">
                                {{ $row['weighing_alert_status'] }}
                            </x-filament::badge>
                    ...
                    <dl class="mt-4 grid grid-cols-2 gap-3 text-sm">
                        <div>
                            ...
                            <dd class="font-medium text-gray-950 dark:text-white">
                                {{ $row['current_head_count'] }} / {{ $row['initial_head_count'] }} ekor
                            </dd>
                        </div>
                        <div>
                        ...
                        <div>
                            <dt class="text-gray-500 dark:text-gray-400">Timbang Terakhir</dt>
                            <dd class="font-medium text-gray-950 dark:text-white">
                                {{ $row['days_since_last_weighing'] ? $row['days_since_last_weighing'].' hari lalu' : '-' }}
                            </dd>
                        </div>
                        <div>
                    ...
                    <dl class="mt-4 grid grid-cols-2 gap-3 border-t border-gray-100 pt-4 text-sm dark:border-gray-800">
                        ...
                        <div>
                            <dt class="text-gray-500 dark:text-gray-400">Naik Total</dt>
                    ...
                    <p class="mt-4 rounded-lg bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                        {{ $row['recommendation'] }}
                        <span class="mt-1 block text-gray-500 dark:text-gray-400">{{ $row['weighing_alert_description'] }}</span>
        ...
        <div class="sickas-table-card hidden overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 lg:block">
            <div class="sickas-table-scroll">
                <table class="sickas-table min-w-[1800px] divide-y divide-gray-200 text-sm dark:divide-gray-800">
                    <thead class="bg-gray-50 text-left text-xs font-semibold uppercase text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                        <tr>
                            <th class="px-4 py-3">{{ $this->monitoringMode === 'individual' ? 'Ternak' : 'Batch' }}</th>
                            <th class="px-4 py-3">Jenis Ternak</th>
                            <th class="px-4 py-3">Mulai</th>
                            <th class="px-4 py-3">Kandang</th>
                            <th class="px-4 py-3 text-right">Ternak</th>
                            <th class="px-4 py-3 text-right">Berat Awal</th>
                            <th class="px-4 py-3 text-right">Berat Terakhir</th>
                            <th class="px-4 py-3 text-right">Naik Total</th>
                            <th class="px-4 py-3 text-right">Rata-rata Awal</th>
                            <th class="px-4 py-3 text-right">Rata-rata Terakhir</th>
                            <th class="px-4 py-3 text-right">Naik Rata-rata</th>
                            <th class="px-4 py-3 text-right">Hari</th>
                            <th class="px-4 py-3 text-right">ADG</th>
                            <th class="px-4 py-3">Status</th>
                            <th class="px-4 py-3">Peringatan Timbang</th>
                        ...
                        @forelse ($this->getRows() as $row)
                            <tr>
                                ...
                                <td class="whitespace-nowrap px-4 py-3 text-right text-gray-700 dark:text-gray-200">
                                    {{ $row['current_head_count'] }} / {{ $row['initial_head_count'] }}
                                </td>
                                ...
                                <td class="whitespace-nowrap px-4 py-3">
                                    ...
                                    <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        {{ $row['days_since_last_weighing'] ? $row['days_since_last_weighing'].' hari lalu' : $row['weighing_alert_description'] }}
                                    </div>
                                </td>
                                ...
                                <td class="min-w-64 px-4 py-3 text-gray-700 dark:text-gray-200">
                                    {{ str($row['recommendation'])->limit(90) }}
                                    <span class="mt-1 block text-xs text-gray-500 dark:text-gray-400">{{ $row['weighing_alert_description'] }}</span>
